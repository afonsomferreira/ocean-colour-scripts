...
#%% Adjust time to weekly
chl_weekly = np.empty((len(lat), len(lon), 1320)) #adjust according to weekly size
#size_test = pd.Series(data=chl[0, 0, :], index=time)
#len(size_test.resample('W').mean())
...
for i in range(len(lat)):
    ...
    for j in range(len(lon)):
        ...
        # Average daily
        pixel_weekly = pixel_ori_pd.resample('W').mean()
        # Add to 3D array
        chl_weekly[i,j,:] = pixel_weekly
        if (i == 0) & (j == 0):
            time_weekly = pixel_weekly.index.values
#%% If you want to run a moving mean or interpolate, do it here
#%% Extract time from datetime
time_weekly_years = pd.to_datetime(time_weekly).year.values
...
for i in range(len(lat)):
    ...
    for j in range(len(lon)):
        for k, yearchar in enumerate(range(1999, 2023)):
            ...
            idx_year_chl = np.argwhere(time_weekly_years == yearchar).ravel()
            # create pandas
            chl_pixel = pd.Series(chl_weekly[i,j, :], index=time_weekly)
            ...
            apr_year_arg = np.argwhere([(chl_pixel.index.month == 4) & (chl_pixel.index.year == yearchar)])[-1][1]+1
            ...
            blooms_dur[blooms_dur < 2] = 0 # checks if blooms have duration below 2 weeks and excludes them
            ...
            for k, item in enumerate(blooms_ind):
                if chl_pixel_sepapr.index[item[0]] <= max_index < chl_pixel_sepapr.index[item[1]]: # checks if the bloom includes the yearly maximum. Otherwise, it is not the main bloom of the year and the algorithm moves on
                    b_start = chl_pixel_sepapr.index[item[0]].weekofyear #calculates bloom initiation day of the year
                    b_end = chl_pixel_sepapr.index[item[1]].weekofyear #calculates bloom termination day of the year
...
# Save!
np.savez_compressed('phenologymetrics_usingweekly',
