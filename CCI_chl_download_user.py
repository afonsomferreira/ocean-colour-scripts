...
def define_ROI(lat_upper, lat_lower, lon_upper, lon_lower):
    ...
    if LATBD[0] <= LATBD[1] or LONBD[1] <= LONBD[0]:
        print("ERROR: Please check that your latitude/longitude values are correct")
    print('Your region of interest is:')
    print('Latitude:', LATBD)
    print('Longitude:', LONBD)
    check_roi = input("Are you sure? [Y/N] ")
    if check_roi != 'Y':
...
def define_time(time_init, time_final):
    ...
    check_time = input("Are you sure? [Y/N] ")
    if check_time != 'Y':
        sys.exit("ERROR: Please repeat")
...
### Define ROI
lat_max = input("Please enter upper right corner latitude [-90-90°N]: ")
lat_min = input("Please enter lower left corner latitude [-90-90°N]: ")
lon_max = input("Please enter upper right corner longitude [-180-180°E]: ")
lon_min = input("Please enter lower left corner longitude [-180-180°E]: ")
LATBD, LONBD = define_ROI(lat_max, lat_min, lon_max, lon_min)
### Define timespan
time_start = input("Please enter initial day [YYYY-MM-DD]: ")
time_end = input("Please enter final day [YYYY-MM-DD]: ")
time_start_datetime, time_end_datetime = define_time(time_start, time_end)
### Download data
filename_out_chl = input("How do you want to name the downloaded file?")
